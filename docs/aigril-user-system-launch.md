# AIGRIL User System Launch Plan

This document defines the production user, membership, payment, and API-access system for AIGRIL.
Do not store API keys, Stripe secrets, webhook secrets, or customer payment details in this file.

## Product Boundary

AIGRIL has two layers:
...
```text
APP_SESSION_COOKIE_NAME=aigril_session
...
STRIPE_RETURN_URL=https://your-frontend-domain.example/about-aigl.html?payment=return&session_id={CHECKOUT_SESSION_ID}
STRIPE_CUSTOMER_PORTAL_RETURN_URL=https://your-frontend-domain.example/about-aigl.html
...
- For best cookie reliability, put frontend and backend under the same site, for example `aigril.com` and `api.aigril.com`.
