...
| Direct MCP tool ids | Implemented as `mcp:<server>:<tool>`; the registry converts them to MCP `tools/call`. |
...
5. Model must choose either mcp_bridge or mcp:<server>:<tool>.
...
3. Promoted MCP-discovered tools into direct runtime call ids: `mcp:<server>:<tool>`.
